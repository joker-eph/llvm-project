...
struct LazyLoadingPass : public PassWrapper<LazyLoadingPass, OperationPass<>> {
  ...
  void runOnOperation() override {
    ...
    std::string bytecode;
    {
      ...
      BytecodeWriterResult result = writeBytecodeToFile(op, os, config);
      if (version >= 0 && result.minVersion != version) {
        op->emitError() << "Failed to write bytecode at version "
                        << (int)version << ", emitted minimum version "
                        << result.minVersion << " instead.";
        signalPassFailure();
        return;
      }
    }
    ...
    while (Operation *op = reader.getNextMaterializableOp()) {
      llvm::outs() << "\n\nBefore Materializing...\n\n";
      op->print(llvm::outs());
      llvm::outs() << "\n\nMaterializing...\n\n";
      if (failed(reader.materialize(op))) {
        op->emitError() << "Failed to materialize";
        signalPassFailure();
        return;
      }
      op->print(llvm::outs());
