...
`RegionBranchOpInterface` exit path for every intermediate level. The exit count
is encoded as a property on the concrete terminator op (e.g. the
`breakingRegionCount` property on `scf.break` and `scf.continue`) and exposed
through the `BreakingTerminatorOpInterface::getNumBreakingRegions()` method.
Custom assembly formats surface the count as a literal integer (e.g.
`scf.break 2`).

The breaking-region count `N` means the operation exits **N region levels** in
...
```mlir
// scf.yield is the standard region terminator (breaking-region count = 1).
