...
* caches each provider's generated-header closure.

Only marked generator targets are added as compilation prerequisites. Library
targets and archives are never added by this mechanism, so mutually linked
static libraries do not form strong build cycles. Disabled generator-expression
arms may generate extra headers, but cannot introduce a library or object
cycle. Imported libraries, paths, and flags require no build-tree ordering.
