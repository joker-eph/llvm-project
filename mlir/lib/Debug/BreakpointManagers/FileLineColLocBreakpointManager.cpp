...
FileLineColLocBreakpoint::parseFromString(
    StringRef str, llvm::function_ref<void(StringRef)> diag) {
  ...
  if (!lineStr.empty() && lineStr.getAsInteger(0, line)) {
    if (diag) {
      ...
      os << "error: initializing FileLineColLocBreakpoint with a non-numeric "
            "line value: `"
         << lineStr << "`";
      diag(os.str());
    }
