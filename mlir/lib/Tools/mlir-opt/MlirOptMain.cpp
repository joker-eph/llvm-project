...
LogicalResult mlir::MlirOptMain(raw_ostream &outputStream,
  ...
  return MlirOptMain(
      outputStream, std::move(buffer), registry,
      MlirOptMainConfig{}
          .setSplitInputFile(splitInputFile)
          .setVerifyDiagnostics(verifyDiagnostics)
          .setVerifyPasses(verifyPasses)
          .setAllowUnregisteredDialects(allowUnregisteredDialects)
          .setPreloadDialectsInContext(preloadDialectsInContext)
          .setEmitBytecode(emitBytecode)
          .setUseImplicitModule(implicitModule)
